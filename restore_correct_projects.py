...
def restore_correct_projects():
    with open('index_with_carousel.html', 'r') as f:
        content = f.read()
    
    # The correct projects data from the working commit
    correct_projects = """        const projects = {
  "2017": [
    {
      "title": "LA ALL STAR",
      "date": "2017.02.18",
  ...
  "2025": [
    ...
    },
    {
      ...
      "images": [
        ...
        // Create image to project mapping
        const imageToProjectMap = {};
        Object.keys(projects).forEach(year => {
            projects[year].forEach(project => {
                project.images.forEach(imageNum => {
                    imageToProjectMap[imageNum] = project;
                });
            });
        });"""
    
    # Replace the projects data and mapping
    import re
    
    # Find and replace the projects data
    pattern = r'const projects = \{[^}]*\};[\s\S]*?const imageToProjectMap = \{\};[\s\S]*?\}\);'
    if re.search(pattern, content):
        content = re.sub(pattern, correct_projects.strip(), content, flags=re.DOTALL)
    
    # Write the corrected content
    with open('index_with_carousel.html', 'w') as f:
        f.write(content)
    
    print("Restored the correct project mapping!")

if __name__ == "__main__":
    restore_correct_projects()
