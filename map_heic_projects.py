#!/usr/bin/env python3
"""
Map Correct Projects with HEIC Support
Process HEIC files from WeTransfer folders and map to correct projects
"""
...
def create_quality_resize(input_path, output_path, target_size):
    """Create high-quality resize with HEIC support"""
    try:
...
def get_project_photos(folder_name):
    """Get photos from a project folder - now including HEIC"""
    ...
    photo_extensions = {'.jpg', '.jpeg', '.heic', '.png', '.tiff', '.tif'}
    photos = []
...
def copy_project_photos_with_heic():
    """Copy actual project photos including HEIC files to correct numbered positions"""
    
    print("🔄 MAPPING CORRECT PROJECT PHOTOS (WITH HEIC SUPPORT)")
    print("=" * 60)
    ...
    print(f"🎯 Projects with real WeTransfer photos: {projects_with_real_photos}")
    return project_mapping
...
if __name__ == "__main__":
    # First copy the correct photos (now with HEIC support)
    copy_project_photos_with_heic()
    
    # Then update the HTML
    if update_html_with_correct_mapping():
        print("\\n🎯 HTML UPDATED WITH CORRECT PROJECT MAPPING!")
        print("✅ Projects now show their actual HEIC photos converted to high-quality JPG!")
    else:
